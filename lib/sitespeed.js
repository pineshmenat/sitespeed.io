...
module.exports = {
  async run(options) {
    ...
    const runningPlugins = await loader.loadPlugins(pluginNames); //require all plugins
    const urlSources = [options.multi ? scriptSource : urlSource];//core plugin
    const allPlugins = urlSources.concat(runningPlugins); //add urlSources to allplugins
    const queueHandler = new QueueHandler(runningPlugins, options); //One Queue for each plugin with plugin added to their queue also
